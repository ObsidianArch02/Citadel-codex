...
via the Codex runtime adapter in `runtimes/codex/generators/install-hooks.js`,
...
Protocol files that load into Codex context on demand.
Built-in skills live in the repository `skills/` directory. Project-level projected skills live under `.agents/skills/`.

```
skills/{name}/SKILL.md                 # Canonical built-in
.agents/skills/{name}/agents/openai.yaml  # Projected runtime artefact
...
`.codex/config.toml` stores project-specific settings:
...
Generated by `node scripts/setup-codex.js --mode full`. Edit manually to customize.
